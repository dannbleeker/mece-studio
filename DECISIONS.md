...
| D8 | Sibling-overlap heuristic (M2) | Token-based: siblings sharing a content word (≥4 chars, common + scaffold nouns stop-worded) → "possible overlap" warn, else "unknown" | Deliberately conservative — it won't catch semantic overlap (synonyms); that's the tier-3 AI judge's job. Tune the stopword list / threshold if it's noisy. |
