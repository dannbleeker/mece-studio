...
describe('opmlToDoc', () => {
  it('builds a tree from a nested OPML outline', () => {
    const doc = opmlToDoc(OPML, 1, 'Imported outline');
  ...
  it('returns null when there are no outlines to import', () => {
    expect(opmlToDoc('<opml><body></body></opml>', 1, 'Imported outline')).toBeNull();
    expect(opmlToDoc('not xml at all', 1, 'Imported outline')).toBeNull();
  });
  ...
  it('caps a pathologically large OPML at the node limit', () => {
    ...
    const doc = opmlToDoc(big, 1, 'Imported outline');
    expect(doc).not.toBeNull();
    if (!doc) return;
