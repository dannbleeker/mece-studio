...
export function StartHome({
  ...
  return (
    <div className="space-y-10">
      <section>
        ...
        <div className="mt-4 flex flex-col gap-2 sm:flex-row">
          <input
            ...
            placeholder={m.start.questionPlaceholder}
            aria-label={m.start.questionLabel}
        ...
        <div className="mt-2.5 flex flex-wrap items-center gap-2">
          ...
          {m.start.sampleQuestions.map((sample) => (
            <button
              key={sample}
              type="button"
      ...
      <section>
        <h2 className={GROUP_HEADING}>{m.start.frameworksHeading}</h2>
        <FrameworksGroup onPick={onPickFramework} />
      </section>

      <section>
        <h2 className={GROUP_HEADING}>{m.start.exampleTreesHeading}</h2>
      ...
      {resume && (
        <section>
          <div className="mb-3 flex items-baseline justify-between gap-3">
            <h2 className={`${GROUP_HEADING} mb-0`}>{m.start.resumeHeading}</h2>
            <div className="flex items-baseline gap-3">
              {reviewCount > 0 && (
                <span className="text-[12px] text-[#bd842c]">
                  {m.start.reviewNeeded({ count: reviewCount })}
                </span>
              )}
              {rest.length > 0 && (
                <button
          ...
          >
            ...
            <span className="min-w-0 flex-1">
              <span className="block truncate font-medium text-[15px] text-neutral-800">
                {docName(resume.doc, m.content.untitledTree)}
              </span>
              <span className="mt-0.5 block text-[12px] text-neutral-500">
                {m.start.treeMeta({
                  kind: treeKind(resume.doc, m),
                  edited: relativeTime(locale, resume.doc.updatedAt),
                })}
              </span>
            ...
            <span className="shrink-0 pr-1 font-medium text-[#3f6fb0] text-[13px]">
              {m.start.resume}
            </span>
          </button>
