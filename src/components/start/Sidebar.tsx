...
export function Sidebar({ section, onSection, onNew, treeCount, reviewCount }: SidebarProps) {
  const badge: Partial<Record<Section, number>> = { all: treeCount, review: reviewCount };
  return (
    <aside className="flex w-[228px] shrink-0 flex-col gap-3 border-[#e7e4dc] border-r bg-[#f4f2ec] p-3">
      <button
        type="button"
        onClick={() => onSection('start')}
