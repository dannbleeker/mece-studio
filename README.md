...
- **Installs, and works with no network.** Install it from the browser and it runs as a desktop / home-screen app. The shell is precached by a service worker, so a tree opens with no connection and a quiet **Offline** notice tells you the network is gone rather than the app being broken. The app asks the browser for **persistent storage** so your trees aren't evicted under storage pressure, checks at every boot that the offline cache is actually intact — repairing it once if it isn't — and reports all of it in **About → offline readiness** (worker state, precache entries, persisted, cached size). A new version never reloads on top of you: it waits behind a **Refresh now** prompt.
