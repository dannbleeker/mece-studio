...
## Tooling backlog
- **Reclaim eager bundle budget by lazy-loading `Workspace`.** `App.tsx` imports both `StartPage` and `Workspace` (the React Flow + dagre canvas) into one eager chunk, so the canvas core counts against first paint even on the Start landing. Since `view` is `start` | `workspace` (mutually exclusive, Start is the default), `React.lazy(() => Workspace)` behind a light Suspense fallback would move the heavy canvas off the cold-start path and free a large chunk of the eager budget (which the template library nudged from 165 → 168 KB). Keep `StartPage` eager (it's the landing). Mind the `Workspace` unit tests + the e2e that lands on the canvas.
- **Extend the Playwright e2e suite.** Covered (`pnpm e2e`, also a CI job): drag-to-reparent + drop-target highlight, inline editing (double-click / Enter / Tab), units, notes, collapse/expand, search, Open-JSON import, export (PNG/PDF/PPTX), AI-prompt copy, the multi-document picker (create / switch / delete), loading an example, the keyboard-shortcuts overlay, and the MECE review dock (flag a split → locate → one-click remedy). The real-input paths are now well covered; extend opportunistically as new UI lands.
